...
export class TasksService implements OnApplicationBootstrap {
    ...
    public static UPDATE_ONIONOO_RELAYS_FLOW: FlowJob = {
        ...
        children: [{ 
            name: 'update-onionoo-relays-validate', queueName: 'onionoo-queue',
            children: [
                { name: 'update-onionoo-relays-fetch', queueName: 'onionoo-queue' }
            ],
        }]
    };
    ...
    private async bootstrap(): Promise<void> {
        ...
        await this.tasksQueue.obliterate();
        await this.onionooQueue.obliterate();
        await this.requestUpdateOnionooRelays(0);
    }

    public async requestUpdateOnionooRelays(delayJob: number = 1000 * 10): Promise<void> {
        await this.tasksQueue.add('update-onionoo-relays', {}, { delay: delayJob });
    }
}
